...
impl Component<Point> for Collision {
    ...
    ///
    /// Adjusts the position of `me` to sit on top of `other`
    /// and nullifies any downward velocity to simulate a solid collision response.
    ///
    /// # Parameters
    ///
    /// - `me`: The Point that owns this Collision component (usually the moving object).
    /// - `other`: The Point with which `me` has collided (usually a static platform or ground).
    fn on_collide(&mut self, me: &mut Point, other: &mut Point) {
        // Calculate distance between points
        let dx = other.position.0;
        let dy = other.position.1;
...
impl Component<Quad> for Collision {
    ...
    fn on_collide(&mut self, me: &mut Quad, other: &mut Quad) {
        ...
        let distance = (dx * dx + dy * dy).sqrt();
        let min_distance = (me.size.0 + other.size.0) * 0.5; // Use size instead of radius
